...
func formatAtom(v reflect.Value) string {
	...
	case reflect.Struct:
		...
		for i := 0; i < v.NumField(); i++ {
			...
			buf.WriteString(fmt.Sprintf("%q: %s", v.Type().Field(i).Name, formatAtom(v.Field(i))))
		}
		buf.WriteRune('}')
		return buf.String()
...
func display(path string, v reflect.Value, step int) {
	...
	case reflect.Slice, reflect.Array:
		for i := 0; i < v.Len(); i++ {
			display(fmt.Sprintf("%s[%d]", path, i), v.Index(i), step+1)
		}
	case reflect.Struct:
		for i := 0; i < v.NumField(); i++ {
			fieldPath := fmt.Sprintf("%s.%s", path, v.Type().Field(i).Name)
			display(fieldPath, v.Field(i), step+1)
		}
	case reflect.Map:
